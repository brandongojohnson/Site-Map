...
// uid). Alongside the study record itself, we write a users/{uid}/studies/
// index entry so "my studies" can be listed without scanning every study.
export const createStudy = async (study, ownerId) => {
  const studyRef = push(ref(db, 'cardSortStudies'));
  const id = studyRef.key;
  const record = { ...study, id, ownerId, createdAt: new Date().toISOString() };
  await update(ref(db), {
    [`cardSortStudies/${id}`]: record,
    [`users/${ownerId}/studies/${id}`]: true,
  });
  return record;
};

// Firebase Realtime Database drops empty arrays on write (they read back as
// undefined), which matters here because open sorts are created with
// categories: []. Normalize on read so consumers can always assume arrays.
export const getStudy = async (studyId) => {
  const snapshot = await get(ref(db, `cardSortStudies/${studyId}`));
  const study = snapshot.val();
  if (!study) return study;
  return { ...study, cards: study.cards || [], categories: study.categories || [] };
};

export const deleteStudy = async (studyId, ownerId) => {
  await update(ref(db), {
    [`cardSortStudies/${studyId}`]: null,
    [`cardSortStudySessions/${studyId}`]: null,
    [`users/${ownerId}/studies/${studyId}`]: null,
  });
};

// All studies created by one account.
export const listMyStudies = async (ownerId) => {
  const snapshot = await get(ref(db, `users/${ownerId}/studies`));
...
export const saveStudySession = async (studyId, session) => {
  const sessionRef = push(ref(db, `cardSortStudySessions/${studyId}`));
...
export const subscribeStudySessions = (studyId, callback) => {
  const sessionsRef = ref(db, `cardSortStudySessions/${studyId}`);
