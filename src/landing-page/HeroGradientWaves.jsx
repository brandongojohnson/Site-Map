...
import './HeroGradientWaves.css';

// Raymarched plasma-wave shader, adapted from reactbits.dev's GradientWaves
// background (https://reactbits.dev/backgrounds/gradient-waves). Used as the
// hero background in light mode only — dark mode keeps the flat solid color.
...
const HeroGradientWaves = ({
  ...
  useEffect(() => {
    ...
    // A single well-formed frame rather than a moving one, for visitors who
    // asked for reduced motion — matches HeroCanvas's STATIC_TIME approach.
    const renderStatic = () => {
  ...
  return <div ref={containerRef} className={`hero-gradient-waves ${className}`.trim()} />;
};
