...
const HeroPrism = ({
  ...
  useEffect(() => {
    ...
    // Reduced motion: still shows a well-formed frame, just no rotation and
    // no hover-driven tilt — matches the pattern used by the other hero
    // backgrounds (HeroGradientWaves, HeroGalaxy, HeroLightfall).
    const effectiveTimeScale = reducedMotion ? 0 : timeScale;
    const effectiveAnimationType = reducedMotion && animationType === 'hover' ? '3drotate' : animationType;
    ...
    const RSY = 1;
    const RSZ = 1;
    const TS = Math.max(0, effectiveTimeScale || 0);
    ...
    if (effectiveAnimationType === 'hover') {
      onPointerMove = (e) => {
        onMove(e);
        startRAF();
      };
    ...
    } else if (effectiveAnimationType === '3drotate') {
      program.uniforms.uUseBaseWobble.value = 0;
    } else {
